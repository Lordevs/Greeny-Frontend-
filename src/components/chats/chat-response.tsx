...
const MarkdownCodeBlock = ({ inline, className, children, content }: any) => {
        ...
        while (i < str.length) {
          ...
          while (i < str.length) {
            ...
            if (!inQuote) {
              ...
              if (braceDepth === 0 && bracketDepth === -1) break;
              if (braceDepth === -1) break;
              if (braceDepth === 0 && bracketDepth === 0 && char === ",") break;
            }
            value += char;
            i++;
          }
...
export const ChatResponse: React.FC<ChatResponseProps> = React.memo(({
  userMessage,
  userFile,
  title,
  description,
  ...
  const markdownComponents = React.useMemo(() => ({
    p: (props: any) => <p className="mb-4 last:mb-0 text-base md:text-xl font-medium" {...props} />,
    h1: (props: any) => <h1 className="text-2xl font-bold mb-4 mt-8 text-primary-foreground first:mt-0" {...props} />,
    h2: (props: any) => <h2 className="text-xl font-bold mb-3 mt-6 text-primary-foreground border-b border-primary-foreground/10 pb-2 first:mt-0" {...props} />,
    h3: (props: any) => <h3 className="text-lg font-bold mb-2 mt-4 text-primary-foreground first:mt-0" {...props} />,
    ...
    li: (props: any) => <li className="mb-1 text-base md:text-lg" {...props} />,
    ...
    thead: (props: any) => <thead className="bg-primary-foreground/10" {...props} />,
    th: (props: any) => <th className="p-3 font-bold border-b border-primary-foreground/10" {...props} />,
    td: (props: any) => <td className="p-3 border-b border-primary-foreground/5" {...props} />,
    code: ({ node, inline, className, children, ...props }: any) => {
      const content = String(children).replace(/\n$/, "");
      return <MarkdownCodeBlock inline={inline} className={className} content={content}>{children}</MarkdownCodeBlock>;
    },
  }), []);
    ...
    <div className="max-w-5xl mx-auto space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12 px-2 md:px-0">
        ...
        <div className="flex justify-end pr-0">
          <div className="bg-primary-foreground text-destructive p-4 md:p-5 rounded-2xl rounded-tr-none shadow-lg max-w-[95%] md:max-w-[70%] text-sm md:text-base leading-relaxed animate-in zoom-in-95 duration-300 wrap-break-word whitespace-pre-wrap flex flex-col gap-3">
            {userFile && fileInfo && FileIconComponent && (
              <div className={`flex items-center gap-2 ${fileInfo.bgColor} p-2 rounded-lg border border-white/20`}>
                <FileIconComponent className={`w-5 h-5 ${fileInfo.color}`} />
                <div className="flex flex-col min-w-0">
                  <span className="text-xs font-bold truncate">{userFile.name}</span>
                  <span className="text-[10px] opacity-70">{(userFile.size / 1024).toFixed(1)} KB</span>
                </div>
              </div>
            )}
      ...
      <div className="flex flex-col md:flex-row items-start gap-3 md:gap-4">
        <div className="flex items-center gap-2 md:block">
          ...
          <span className="md:hidden text-xs font-bold text-primary-foreground uppercase tracking-wider">AI Assistant</span>
        </div>
        ...
        <div className="flex-1 w-full relative">
          <div className="bg-secondary p-5 md:p-8 rounded-2xl md:rounded-3xl space-y-5 md:space-y-8 shadow-xl overflow-hidden shadow-black/5 border border-white/5">
            <div className="flex-1">
              <h3 className="text-xl md:text-2xl font-bold text-primary-foreground mb-3 leading-tight">{title}</h3>
            ...
            {isVisualization && plotImage && (
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h4 className="text-xs font-bold text-primary-foreground uppercase tracking-[0.2em]">Visualization</h4>
                ...
                <Card className="overflow-hidden border-none shadow-2xl bg-white/5 backdrop-blur-sm">
                  <img src={`data:image/png;base64,${plotImage}`} alt="Data Visualization" className="w-full h-auto rounded-xl" />
                </Card>
              </div>
            )}
